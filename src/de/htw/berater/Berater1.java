...
public class Berater1 extends Berater {
	...
	private void smartphoneZweck(String string) {
		...
		for (OntClass clazz : smartphonesWithHatZweck) {
			...
			for (Restriction restriction : restrictions) {
				OntClass range = restriction.asSomeValuesFromRestriction()
		...
		if (abstractClassSomewhere) {
			...
			int group = 0;
			rememberList.clear();
			if (classesCoveringAxiomsResolved == null)
				throw new RuntimeException("Ontologie geaendert");
			for (int i = 0; i < classesCoveringAxiomsResolved.size(); i++) {
				...
				for (OntClass smphone : classes) {
					...
					if (i == classesCoveringAxiomsResolved.size() - 1)
						cb.add("Ein " + smphone.getLocalName(),
								smphone.getLocalName(), ChoiceType.CHECK, group);
					else
						cb.add("Ein " + smphone.getLocalName(),
								smphone.getLocalName(), ChoiceType.RADIO, group);
			...
			// dieser fall ist nicht im Szenario enthalten, aber egal.
			// Funktioniert auch für BilderMachenZweck
			for (int i = 0; i < smartphonesWithHatZweckOnZweckKeyword.size(); i++) {
				setCurrentProperties(smartphonesWithHatZweckOnZweckKeyword
						.get(i));
			}
			context = 3;
	...
	private void displaySmartphone(String display) throws Exception {
		OntClass displaySmartphone = searchClassContaining(display,
				"Smartphone");
		if (display != null)
			setCurrentProperties(displaySmartphone);
		if (customer.isCustomer(Customer.SEHBEHINDERT)) {
			if (isSmartphoneOkForCustumer(displaySmartphone,
					Customer.SEHBEHINDERT)) {
				OntClass tastaturSmartphone = searchClassContaining("Tastatur",
						"Smartphone");
				if (!isSmartphoneOkForCustumer(tastaturSmartphone,
						Customer.SEHBEHINDERT)) {
					...
					for (OntClass property : getClassProperties(tastaturSmartphone)) {
						ComplementClass cc = model.createComplementClass(null,
								property);
						complements.add(cc);
					}
					RDFList inList = model.createList(complements
							.toArray(new RDFNode[0]));
					OntClass intersectionClass = model.createIntersectionClass(
							null, inList);
					tmpClass.addSuperClass(intersectionClass);
					setCurrentProperties(tmpClass);

					System.out
					.println("es wird eine Frage übersprungen: Möchten Sie ein reines Touchdisplay oder eine zusätzliche Hardwaretastatur?");
					context = 5;
				...
				context = 3;
				throw new Exception(
						"Sie können kein kleines Smartphone wählen, wenn Sie sehbehindert sind. Daher wiederhole ich die letzte Frage nochmal");
			}
		}
		context = 4;
		nextQuestion = new Question(
				"Möchten Sie ein reines Touchdisplay oder eine zusätzliche Hardwaretastatur?",
				ChoicesBuilder.yesNo("Ein Gerät mit integrierter Tastatur",
						"Ein Gerät ohne Hardware-Tastatur"));
	}

	private void touchBedinung(boolean withKeyboard) {
		List<OntClass> properties = new LinkedList<OntClass>();
		OntClass subClassOfInterest = searchClassContaining("Tastatur",
				"Smartphone");
		...
		if (!withKeyboard) {
			...
			for (OntClass property : properties) {
				ComplementClass cc = model
						.createComplementClass(null, property);
				complements.add(cc);
			}
			RDFList inList = model.createList(complements
					.toArray(new RDFNode[0]));
			OntClass intersectionClass = model.createIntersectionClass(null,
					inList);
			tmpClass.addSuperClass(intersectionClass);
		} else {
			RDFList inList = model.createList(properties
					.toArray(new RDFNode[0]));
	...
	private void outdoorSmartphone(String outdoor) throws DBException {
		...
		if (outdoor.equals("Outdoor")) {
			List<OntClass> properties = setCurrentProperties(outdoorSmartphones
					.get(0));
			...
			for (OntClass x : result) {
				...
				String two = outdoorSmartphones.get(0).getSubClass()
						.getLocalName();
				if (one.contains(two)) {
					result.remove(x);
				}
			...
			context = 6;
			nextQuestion = new Question("Möchten Sie ein "
					+ result.get(0).getLocalName() + "?", ChoicesBuilder.yesNo(
							"Ja", "Nein"));
		} else {
			if (outdoor.equals("Business")) {
				OntClass smartphone = searchClassContaining("Business",
						"Smartphone");
				if (smartphone != null) {
	...
	private void searchForCustomer(String customer, OntClass property)
			throws DBException {
		if (property.isRestriction()) {
			...
			if (property.isIntersectionClass()) {
				for (Iterator<? extends OntClass> it = property
						.asIntersectionClass().listOperands(); it.hasNext();) {
					OntClass op = it.next();
				...
				if (property.isUnionClass()) {
					for (Iterator<? extends OntClass> it = property
							.asUnionClass().listOperands(); it.hasNext();) {
						OntClass op = it.next();
						searchForCustomer(customer, op);
					}
				} else {
					if (property.isComplementClass()) {
						for (Iterator<? extends OntClass> it = property
								.asComplementClass().listOperands(); it
								.hasNext();) {
							OntClass op = it.next();
							searchForCustomer(customer, op);
						}
	...
	private void kameraSmartphone(String answer) {
		if (answer.indexOf("GuteKamera") != -1) {
			...
			List<OntClass> kamera = findClassEquals(answer.toLowerCase()
					+ "smartphone");
			setProperties(kamera);
		}
		context = 8;
		...
		nextQuestion = new Question(
				"Bevorzugen Sie einen bestimmten Hersteller?", choicesGroupMap);
	}
	...
	public Question firstSpecificQuestion() {
		...
		List<Choice> choices = zweckClass.listSubClasses()
				.mapWith(new Map1<OntClass, Choice>() {
					...
					public Choice map1(OntClass o) {
						...
						String text = value.replace("Zweck", "").replaceAll(
								"([^A-Z])([A-Z])", "$1 $2");
		...
		return new Question("Für welchen Zweck benötigen Sie ein Smartphone?",
				choicesMap);
	}
	...
	private Question questionCamera() {
		...
		.add("Ja, ich brauche eine gute Kamera.", "GuteKamera",
				ChoiceType.RADIO)
				.add("Nein, eine Kamera brauch ich nicht.", "Nein",
						ChoiceType.RADIO).build();
		return new Question("Nutzen Sie das Smartphone auch als Kamera?",
				choices);
	}
